...
from Lunchpad.src.modules.encoder import EncoderCNN, EncoderLabels
from Lunchpad.src.modules.multihead_attention import MultiheadAttention
from Lunchpad.src.modules.transformer_decoder import DecoderTransformer
from Lunchpad.src.utils.metrics import MaskedCrossEntropyCriterion, softIoU
...
def get_model(args, ingr_vocab_size, instrs_vocab_size):
    ...
    encoder_ingrs = EncoderLabels(args.embed_size, ingr_vocab_size, args.dropout_encoder, scale_grad=False).to(device)
    # build image model
...
class InverseCookingModel(nn.Module):
    ...
    def forward(self, img_inputs, captions, target_ingrs, sample=False, keep_cnn_gradients=False):
        if sample:
        ...
        if not self.recipe_only:
            ...
            losses["eos_loss"] = mult * (eos_loss * eos_pos.float()).sum(1) / (eos_pos.float().sum(1) + 1e-6) + mult * (
                eos_loss * eos_head.float()
            ).sum(1) / (eos_head.float().sum(1) + 1e-6)
            # iou
