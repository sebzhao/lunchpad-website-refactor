...
class Recipe1MDataset(data.Dataset):
    def __init__(
        ...
        if use_lmdb:
            self.image_file = lmdb.open(
                os.path.join(aux_data_dir, "lmdb_" + split),
                max_readers=1,
                readonly=True,
                lock=False,
                readahead=False,
    ...
    def get_ingrs_vocab(self):
        return [
            min(w, key=len) if not isinstance(w, str) else w for w in self.ingrs_vocab.idx2word.values()
        ]  # includes 'pad' ingredient
