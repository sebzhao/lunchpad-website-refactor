...
from Lunchpad.src.build_vocab import Vocabulary
from Lunchpad.src.data_loader import get_loader
from Lunchpad.src.model import get_model, label2onehot, mask_from_eos
from Lunchpad.src.utils.metrics import compute_metrics, softIoU, update_error_types
from Lunchpad.src.utils.tb_visualizer import Visualizer
...
def merge_models(args, model, ingr_vocab_size, instrs_vocab_size):
    load_args = pickle.load(
        open(os.path.join(args.save_dir, args.project_name, args.transfer_from, "checkpoints/args.pkl"), "rb")
    )
...
def main(args):
    ...
    if args.ingrs_only:
        ...
        params = (
            list(model.recipe_decoder.parameters())
    ...
    if params_cnn is not None and args.finetune_after == 0:
        optimizer = torch.optim.Adam(
            [{"params": params}, {"params": params_cnn, "lr": args.learning_rate * args.scale_learning_rate_cnn}],
    ...
    for epoch in range(start, args.num_epochs):
        ...
        if (
            args.finetune_after != -1
            and args.finetune_after < epoch
            and not keep_cnn_gradients
            and params_cnn is not None
        ):
        ...
        for split in ["train", "val"]:
            ...
            for i in range(total_step):
                ...
                loss = (
                    args.loss_weight[0] * recipe_loss
            ...
            if args.tensorboard:
                ...
                logger.scalar_summary(
                    mode=split, epoch=epoch, **{k: np.mean(v) for k, v in total_loss_dict.items() if v}
                )
