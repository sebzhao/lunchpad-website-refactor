...
from Lunchpad.src.model import get_model, mask_from_eos
...
def main(args):
                ...
                if args.get_perplexity:
                    ...
                    if not args.recipe_only:
                        ...
                        for ingr_idx, fake_ingr in enumerate(fake_ingrs):
                            iou_item = softIoU(
                                pred_one_hot[ingr_idx].unsqueeze(0), target_one_hot[ingr_idx].unsqueeze(0)
                            ).item()
    ...
    if args.get_perplexity:
        ...
        if not args.recipe_only:
            ...
            compute_metrics(
                ret_metrics, error_types, ["accuracy", "f1", "jaccard", "f1_ingredients"], eps=1e-10, weights=None
            )
