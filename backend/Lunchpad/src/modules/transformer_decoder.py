...
import Lunchpad.src.modules.utils as utils
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
from Lunchpad.src.modules.multihead_attention import MultiheadAttention
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x, ingr_features, ingr_mask, incremental_state, img_features):
        ...
        if ingr_features is None:
            ...
            mask = torch.cat(
                (torch.zeros(img_features.shape[1], img_features.shape[0], dtype=torch.uint8).to(device), ingr_mask), 1
            )
            x, _ = self.cond_att(
                query=x,
                key=kv,
...
class DecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.embed_tokens = nn.Embedding(
            vocab_size, embed_size, padding_idx=vocab_size - 1, scale_grad_by_freq=scale_embed_grad
        )
        ...
        self.layers.extend(
            [
                TransformerDecoderLayer(
                    embed_size, attention_nheads, dropout=dropout, normalize_before=normalize_before, last_ln=last_ln
                )
    ...
    def sample(
        ...
        if beam != -1:
            if fs == 1:
                return self.sample_beam(
                    ingr_features, ingr_mask, beam, img_features, first_token_value, replacement, last_token_value
                )
            else:
        ...
        for i in range(self.seq_length):
            ...
            outputs, _ = self.forward(
                ingr_features, ingr_mask, torch.stack(sampled_ids, 1), img_features, incremental_state
            )
    ...
    def sample_beam(
        ...
        for i in range(self.seq_length):
            ...
            for rem in range(len(sequences)):
                ...
                outputs, _ = self.forward(
                    ingr_features, ingr_mask, torch.stack(sequences[rem][0], 1), img_features, incremental
                )
                outputs = outputs.squeeze(1)
                if not replacement:
                    # predicted mask
            ...
            ordered_all = sorted(
                all_candidates + finished, key=lambda tup: tup[1] / (np.power(tup[-1], alpha)), reverse=True
            )[:k]
